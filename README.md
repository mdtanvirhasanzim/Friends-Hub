...
## FriendsHub authentication setup (no email confirmation)

For the requested flow — create account -> immediately enter FriendsHub — disable email confirmation in Supabase: **Authentication -> Providers -> Email -> Confirm email = OFF**. PostgreSQL RLS cannot disable Supabase Auth email confirmation.

Then run `supabase_member_access.sql` in the Supabase SQL Editor. The migration gives authenticated members read access across the public tables and keeps profile self-update/insert access.

The frontend now treats a missing sign-up session as a configuration error instead of pretending the account is fully logged in. When email confirmation is disabled, Supabase returns a session, the AuthContext sets `currentUser`, and `App.tsx` automatically renders the home application instead of the login screen.
